...
| Architecture reviewer| *(one of three reviews spawned together and run in parallel, each in its own read-only worktree)* Design & architecture audit of the cycle diff — dependency rule, ports and adapters, information hiding, SOLID — measured against the project article's own architecture rules and driven through the **Codex CLI** (`gpt-5.5` @ `xhigh`). The pipeline's only structural check. Read-only; findings routed back to the Coder by the lead |
| Security reviewer    | OWASP Top 10 audit of the cycle diff, same Codex reviewer pattern for an independent model's opinion. Read-only; findings routed to the Coder by the lead |
